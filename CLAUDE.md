...
- **Netcode (rollback correctness + full-stack E2E under loss):**
  `dotnet test "sandbox~/Net/tests/Tanks.Net.Tests.csproj"` — the deterministic suite
  asserts confirmed states are hash-identical to a serial reference run; the E2E suite
  runs two whole peers over the in-memory pair (clean + 25% loss w/ duplication).
- **Netcode on UNITY'S runtime, headless:** `-executeMethod Tanks.Editor.NetcodeSmokeTest.Run`
  (same batch-mode shape as the protocol smoke) — a full rollback match over loopback
  NativeTransport inside the Editor's Mono; "NETCODE SMOKE PASSED" + exit 0 = pass.
