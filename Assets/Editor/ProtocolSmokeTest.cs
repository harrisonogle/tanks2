...
namespace Tanks.Editor
{
    ...
    public static class ProtocolSmokeTest
    {
        ...
        private static void RunLoopbackHandshake()
        {
            ...
            using (ta)
            using (tb)
            using (var a = new Network(new NetworkPipeFactory(), ta, TanksPeerCrypto.Instance, NullLogger.Instance, 1, 1))
            using (var b = new Network(new NetworkPipeFactory(), tb, TanksPeerCrypto.Instance, NullLogger.Instance, 1, 1))
            {
                try
                {
                    a.Start();
                    ...
                    WaitFor(a, PipeEventKind.SessionEstablished, "initiator");
                    WaitFor(b, PipeEventKind.SessionEstablished, "responder");

                    a.Disconnect(b.LocalPeerId);
                    WaitFor(b, PipeEventKind.SessionClosed, "responder (close)");
                }
                finally
                {
                    a.Stop();
        ...
        private static void WaitFor(Network network, PipeEventKind kind, string who)
        {
            IPipeReader reader = network.Pipe.RX.Reader;
            ...
            while (sw.Elapsed < HandshakeTimeout)
            {
                ...
                {
                    PipeEventKind seen = evt.Metadata.Kind;
