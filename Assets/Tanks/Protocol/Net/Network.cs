...
public sealed unsafe class Network : INetwork, IDisposable
{
    ...
    {
        public NetworkState(
            Umem umem, IDatagramTransport transport, IPeerCrypto crypto,
        ...
        public readonly UmemPool Rx;
        public readonly UmemPool Tx;
        ...
        int umemSlotCount)
    {
        ...
        ThrowHelper.ThrowIfNegativeOrZero(umemSlotCount);

        if (!BitConverter.IsLittleEndian)
            throw new InvalidOperationException("Big endian is not supported.");

        _logger = logger;
        _umem = new Umem(umemSlotCount, NetworkConstants.ApplicationDataSlotSize);
        _tx = _umem.Tx;
        ...
        _state = new NetworkState(
            _umem, transport, crypto, _localKey, localPeerId,
        ...
        IPEndPoint endpoint)
    {
        ...
        try
        {
            ...
            metadata->Kind = NetworkEventKind.ActiveOpen;
            metadata->Offset = 0;
            metadata->Length = Net.Connect.Size;
            buffer += NetworkConstants.NetworkEventHeadroom;
    ...
    public void Disconnect(PeerId remotePeerId)
    {
        ...
        try
        {
            ...
            var metadata = (NetworkEventMetadata*)buffer;
            metadata->SessionId = default;
            metadata->Kind = NetworkEventKind.ActiveClose;
            metadata->Offset = 0;
            metadata->Length = DisconnectPeer.Size;
            var msg = (DisconnectPeer*)(buffer + NetworkConstants.NetworkEventHeadroom + metadata->Offset);
    ...
    private static void DrainTx(ref NetworkState network)
    {
        ...
        while (budget++ < TxBudget && network.Tx.TryDequeue(ref network.Handle))
        {
            try
            {
                byte* slot = network.Tx.Deref(network.Handle);
                var metadata = (NetworkEventMetadata*)slot;
                SessionProtocol? session;

                switch (metadata->Kind)
                {
                    case NetworkEventKind.ActiveOpen:
                        ActiveOpen(ref network, slot);
                        break;
                    case NetworkEventKind.ActiveClose:
                    ...
                    case NetworkEventKind.ApplicationData:
                        ...
                        {
                            session.TrySendApplicationData(slot, network.Tx.SlotSize);
                        }
                        else
                        {
            ...
            {
                network.Tx.Return(network.Handle);
            }
        }
    }
    ...
    private static bool PollRx(ref NetworkState network)
    {
        ...
        if (!network.Rx.TryRent(ref network.Handle))
        {
        ...
        try
        {
            byte* slot = network.Rx.Deref(network.Handle);
            byte* slotEnd = slot + network.Rx.SlotSize;
            byte* packetBegin = slot + NetworkConstants.NetworkEventHeadroom;
            ...
            else
            {
                ...
                if (!network.Rx.TryEnqueue(network.Handle))
                {
        ...
        finally
        {
            if (!consumedBuffer)
            {
                network.Rx.Abandon(network.Handle);
            }
        }
    }
    ...
    {
        var metadata = (NetworkEventMetadata*)slot;
        Debug.Assert(metadata->Kind == NetworkEventKind.ActiveClose);
        ...
        {
            var msg = (DisconnectPeer*)(slot + NetworkConstants.NetworkEventHeadroom + metadata->Offset);
    ...
    {
        var metadata = (NetworkEventMetadata*)slot;
        Debug.Assert(metadata->Kind == NetworkEventKind.ActiveOpen);
        ...
        var session = new SessionProtocol(
            network.TxBuffer, network.TxBufferLength, ctx, network.Transport, network.Umem, network.Logger, network.Metrics);
