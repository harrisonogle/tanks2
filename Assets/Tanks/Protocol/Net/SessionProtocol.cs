...
public sealed unsafe class SessionProtocol
{
        ...
        NetworkMetrics metrics)
    {
        ...
        _logger = logger;
        _metrics = metrics;
        _txRing = pipe.TX.Ring;
        _txPool = pipe.TX.Pool;
        _txSlotSize = _txPool.SlotSize;
        _rxRing = pipe.RX.Ring;
        _rxPool = pipe.RX.Pool;
        _rxSlotSize = _rxPool.SlotSize;
    ...
    public bool TrySendApplicationData(byte* slot, int slotSize)
    {
        ...
        Debug.Assert(metadata->Kind == PipeEventKind.ApplicationData);
        ...
        Debug.Assert(packet >= slot + NetworkConstants.PipeEventHeadroom);
    ...
    private void EmitSessionAccepted()
    {
        ...
        ref PipeEventMetadata metadata = ref _lifecycle.SessionAccepted.Metadata;
        ref SessionAccepted payload = ref _lifecycle.SessionAccepted.Payload;

        bool rented = _rxPool.TryRent(ref _buffer);
        if (rented)
        {
            byte* buffer = _rxPool.Deref(_buffer);
            metadata = ref *(PipeEventMetadata*)buffer;
            payload = ref *(SessionAccepted*)(buffer + NetworkConstants.PipeEventHeadroom);
        }

        metadata.SessionId = _ctx.LocalSessionId;
        metadata.Kind = PipeEventKind.SessionAccepted;
        metadata.Offset = 0;
        ...
        if (!_rxRing.TryEnqueue(_buffer))
        {
            _rxPool.Abandon(_buffer);
    ...
    private void EmitSessionEstablished()
    {
        ...
        ref PipeEventMetadata metadata = ref _lifecycle.SessionEstablished.Metadata;
        bool rented = !previousPending && _rxPool.TryRent(ref _buffer);
        if (rented)
        {
            metadata = ref *(PipeEventMetadata*)_rxPool.Deref(_buffer);
        }
        metadata.SessionId = _ctx.LocalSessionId;
        metadata.Kind = PipeEventKind.SessionEstablished;
        metadata.Offset = 0;
        metadata.Length = 0;
        /* (empty payload) */
        if (!rented)
    ...
    private void EmitSessionClosed(DisconnectReason peerReason, EndReason endReason)
    {
        ...
        ref PipeEventMetadata metadata = ref _lifecycle.SessionClosed.Metadata;
        ...
        if (rented)
        {
            byte* buffer = _rxPool.Deref(_buffer);
            metadata = ref *(PipeEventMetadata*)buffer;
            payload = ref *(SessionClosed*)(buffer + NetworkConstants.PipeEventHeadroom);
        }
        metadata.SessionId = _ctx.LocalSessionId;
        metadata.Kind = PipeEventKind.SessionClosed;
        metadata.Offset = 0;
    ...
    private void FlushPendingSessionAccepted()
    {
        ...
        if (!_rxPool.TryRent(ref _buffer)) return;

        byte* slot = _rxPool.Deref(_buffer);
        *(PipeEventMetadata*)slot = _lifecycle.SessionAccepted.Metadata;
        *(SessionAccepted*)(slot + NetworkConstants.PipeEventHeadroom) = _lifecycle.SessionAccepted.Payload;
    ...
    private void FlushPendingSessionEstablished()
    {
        ...
        *(PipeEventMetadata*)_rxPool.Deref(_buffer) = _lifecycle.SessionEstablished.Metadata;
    ...
    private void FlushPendingSessionClosed()
    {
        ...
        byte* slot = _rxPool.Deref(_buffer);
        *(PipeEventMetadata*)slot = _lifecycle.SessionClosed.Metadata;
        *(SessionClosed*)(slot + NetworkConstants.PipeEventHeadroom) = _lifecycle.SessionClosed.Payload;
    ...
    private struct LifecycleEvents
    {
        internal struct SessionAcceptedBlob
        {
            public bool Emitted;
            public bool Pending;
            public PipeEventMetadata Metadata;
