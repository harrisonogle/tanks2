...
using var networkA = new Network(transportA, cryptoA, loggerA, AppProtocolId, AppProtocolVersion, 1 << 10);
using var networkB = new Network(transportB, cryptoB, loggerB, AppProtocolId, AppProtocolVersion, 1 << 10);
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            foreach (var (sessionId, session) in sessions)
            {
                ...
                try
                {
                    ...
                    int remainingLength = tx.SlotSize - (NetworkConstants.NetworkEventHeadroom + PacketHeader.Size);
...
// Minimal game-thread session view for this sample; the real one (PeerState, with
// input/hash rings) lives in Tanks.Net, which this Protocol-only sample doesn't reference.
sealed class SampleSession
{
    private SessionAccepted _accepted;
    public SessionState State { get; private set; }
    public SessionId LocalSessionId => _accepted.LocalSessionId;
    public void OnSessionAccepted(ref SessionAccepted evt) { _accepted = evt; State = SessionState.Init; }
    public void OnSessionEstablished() { State = SessionState.Established; }
    public void OnSessionClosed(ref SessionClosed evt) { State = SessionState.Closed; }
}
