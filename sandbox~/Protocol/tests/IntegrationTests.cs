...
public sealed unsafe class IntegrationTests
{
    ...
    public void SimultaneousOpen_Establishes_AndPayloadsRoundtrip()
    {
        ...
        try
        {
            ...
            Assert.True(TestHelpers.WaitFor(a.Pipe.RX, PipeEventKind.SessionEstablished, HandshakeTimeout,
                (ref PipeEvent evt, byte* _) => sidA = evt.Metadata.SessionId), "A did not establish");
            Assert.True(TestHelpers.WaitFor(b.Pipe.RX, PipeEventKind.SessionEstablished, HandshakeTimeout,
                (ref PipeEvent evt, byte* _) => sidB = evt.Metadata.SessionId), "B did not establish");
            ...
            Assert.True(TestHelpers.WaitFor(b.Pipe.RX, PipeEventKind.ApplicationData, HandshakeTimeout,
                (ref PipeEvent evt, byte* payload) =>
                {
                    ...
                    matched = true;
                    for (int i = 0; i < 32; i++)
                        if (data[i] != (byte)(i * 7)) { matched = false; break; }
    ...
    public void DeadLink_TimesOut_AndDeliversSessionClosed()
    {
        ...
        try
        {
            ...
            Assert.True(TestHelpers.WaitFor(a.Pipe.RX, PipeEventKind.SessionClosed,
            ...
            Assert.True(TestHelpers.WaitFor(b.Pipe.RX, PipeEventKind.SessionClosed,
                NetworkConstants.SessionIdleTimeout + TimeSpan.FromSeconds(10)),
