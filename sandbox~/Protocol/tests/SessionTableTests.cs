...
public sealed unsafe class SessionTableTests
{
    ...
    // RSA keygen is too slow to repeat 100k times. The pipe lifetime is held
    // statically so its finalizer can't free the rings mid-test.
    private static readonly INetworkPipeLifetime s_pipe = new NetworkPipeFactory().Create(slotCount: 64);
    ...
    private static SessionProtocol CreateSession()
    {
        ...
        return new SessionProtocol(null, 0, ctx, transport, s_pipe.Pipe, NullLog.Instance, new NetworkMetrics());
    }
