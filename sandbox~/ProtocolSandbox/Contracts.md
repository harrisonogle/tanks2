...
| Peer protocol thread | App (started at bootstrap) | `PeerProtocol` component |
...
- **Event queue** — produced by `PeerProtocol`, consumed by the game thread. Carries status events and in-session data events. Thread-safe, single-producer / single-consumer.
...
Lives on the game thread. Tracks which UX state the player is in (per `UXFlow.md`) and translates between PeerProtocol's events and the UI's rendering needs. It is the *only* thing on the game thread that calls PeerProtocol's control signals; the UI code calls the UXStateMachine.
...
component UXStateMachine
  ...
  depends on:
    PeerProtocol   (calls control signals; consumes events from the event queue)
...
The UX state machine consumes PeerProtocol's events on each game-thread tick (drain queue → apply transitions). UI code reads `CurrentState` to render the appropriate screen.

## Contracts

### PeerLookup

Read interface to the peer table. Exposed by `Discovery`; consumed by `PeerProtocol`.
...
### PeerSocket and DiscoverySocket
...
```
                ...
                └──────┬───────┘
                       ...
                       ▼
                ┌──────────────┐         ┌──────────────┐
                │ PeerProtocol │────────►│ PeerLookup   │
                └──────┬───────┘ reads   └──────┬───────┘
                       │ UDP                    │ exposed by
                       ▼                        ▼
                ┌──────────────┐         ┌──────────────┐
                │ PeerSocket   │         │ Discovery    │
                └──────────────┘         └──────┬───────┘
                                                │ UDP
                                                ▼
                                         ┌──────────────┐
...
- **Cancellation signals.** `UXFlow.md` defers cancellation of `Searching` / `Connecting`. When added: `CancelMatchmaking()` and `AbortHandshake()` signals on PeerProtocol, plus the matching UI signals on UXStateMachine.
- **Peer selection.** Auto-pick only. If a lobby UI is added later, `PeerProtocol.StartMatchmaking()` would gain a `peerId` parameter (or split into `BeginPeerDiscovery()` + `ConnectToPeer(peerId)`), and the game thread would gain visibility into the peer table via `PeerLookup`.
...
- **Multiple concurrent sessions.** Out of scope — only one peer session at a time. PeerProtocol could be extended to manage N sessions; not modeled here.